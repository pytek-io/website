...
You can pass a hash argument to an app by appending a string at the end of the path using the `#` character as separator. For example invoking the code below at `app/tutorial.hash#John` will yield the following.
...
```read_module
tutorial.hash_update#John
```

We can't display the result in this page directly as it would clash with the website navigation logic. You can instead follow this [link](tutorial.hash_update#John) to launch this app and see how the app path is synchronized with the app content when navigating back and forth.
