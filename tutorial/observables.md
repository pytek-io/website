...
In the previous examples, we created apps whose state is defined by their input components values. Taking this further, we can also create observable values, that is app state variables, by using the `make_observable` method as shown in the example below.

```load_module
tutorial.snippets.make_observable
```
...
You have probably noticed some strong similarities between `ObservableValue` and the input component in that they are both callable and observable. Using the standard [terminology](https://en.wikipedia.org/wiki/Observer_pattern) we will refer to them as observables from now on.

### Creating observable collections

Defining the app state may also require the use of collection types such as `list` or `dict`. One can also pass those to the `make_observable` method which will wrap them within `ObservableList` or `ObservableDict` respectively. Those objects have the same interface as their built-in Python counterparts, except that they are observables. The example below shows a simple user list being populated and displayed.

```load_module
tutorial.snippets.make_observable_collection
```
...
In this example we wrapped `users` within an `ObservableList` called `user_obs`. Note that doing this implies that we only modify it through `user_obs` methods. You can still access `users` directly but in "read only" mode, to persist it, inspect it, etc.
...
As you might expect, you will find a log of all the inputs entered in the browser console. This is particularly useful for debugging but could also be used for auto-saving app states, triggering asynchronous tasks, etc.
